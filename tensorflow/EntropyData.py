...
def DataHandle(WavePrice,WaveVolum,WaveHigh,WaveLow,LenWavePrice,LenWaveVolum):
    ...
    MACD = np.array(talib.MACD(np.array(WavePrice)))[0]
    MACD[0:34] = 0
    MACD = MACD.tolist()
    MOM_3 = [0] * 2
    for i in range(2,LenWavePrice):
...
def ReadDataFromXml(path,state_param= np.pi / 4 , Swing = 1):
    data = pd.read_excel(path,'Sheet0',header=None)
    RawPrice = np.array(data.iloc[1:,6]).tolist()
    RawVolum = np.array(data.iloc[1:,9]).tolist()
    RawHigh = np.array(data.iloc[1:,4]).tolist()
    RawLow = np.array(data.iloc[1:,5]).tolist()
...
def YieldRate(RawPrice,state,All_ind):
    ...
    signal = []
    for i in range(r-1):
        # sig = (np.mean(RawPrice[i+1:i+6])-RawPrice[i]) / RawPrice[i] * 100
        sig = RawPrice[i+1]
        signal.append(sig)
    train_signal1 = []
    train_signal2 = []
    ...
    test_signal = signal[int(len(All_ind) * train_percent):]
...
def ReadDataByTuShare(code=0,start='2017-01-01',end='2017-11-21',ktype='D',autype='qfq',state_param=np.pi / 4,Swing=0.1):
