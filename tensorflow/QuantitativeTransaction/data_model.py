...
class StockDataSet(object):
    def __init__(self,
        ...
        if self.initCsv:
            raw_df = pd.read_excel(os.path.join("data", "%s.xlsx" % self.stock_sym),self.sheet,header=None)
        ...
        raw_df = ts.get_k_data('600230','2000-01-01','2016-12-31','D')
        RawPrice = np.array(raw_df.iloc[:,2]).tolist()
        RawVolum = np.array(raw_df.iloc[:,5]).tolist()
        RawHigh = np.array(raw_df.iloc[:,3]).tolist()
        RawLow = np.array(raw_df.iloc[:,4]).tolist()
    ...
    def GetTestData(self,test_sym):
        ...
        raw_df = ts.get_k_data('600230','2017-01-01','2017-12-31','D')
        RawPrice = np.array(raw_df.iloc[:,2]).tolist()
        RawVolum = np.array(raw_df.iloc[:,5]).tolist()
        RawHigh = np.array(raw_df.iloc[:,3]).tolist()
        RawLow = np.array(raw_df.iloc[:,4]).tolist()
        #斜率
        raw_price = np.append(self.raw_price[-4:],RawPrice,axis=0)
        slop_5 = []
        for i in range(4,len(RawPrice)+4):
            slop_5.append(math.atan(100 * (raw_price[i] - raw_price[i-4]) / raw_price[i-4]) * 180 / np.pi)
        state = np.zeros(len(RawPrice))
        ...
        for i in range(len(RawPrice)+5):
            ...
            if i <4:
                price = self.raw_price[-204 + i:i-4]
                volum = self.raw_volum[-204 + i:i-4]
                high = self.raw_high[-204 + i:i-4]
                low = self.raw_low[-204 + i:i-4]
            elif i == 4:
                price = self.raw_price[-200:]
                volum = self.raw_volum[-200:]
                high = self.raw_high[-200:]
                low = self.raw_low[-200:]
            elif i < 204 and i >4:
                price = self.raw_price[-204 + i:] + RawPrice[:i-4]
                volum = self.raw_volum[-204 + i:] + RawVolum[:i-4]
                high = self.raw_high[-204 + i:] + RawHigh[:i-4]
                low = self.raw_low[-204 + i:] + RawLow[:i-4]
            else:
                price = RawPrice[i-204:i-4]
                volum = RawVolum[i-204:i-4]
                high = RawHigh[i-204:i-4]
                low = RawLow[i-204:i-4]
