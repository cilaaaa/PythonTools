...
#             print(i+len(train_index1)+len(train_index2)+len(train_index3),j)
...
def tf_better_nn(input,output,hidden_node_count,train_x,train_y,layer_cnt=2,keep_prob=0.5,batch_size=100,steps=30000):
    ...
    with graph.as_default():
        ...
        # last weight
        # W2 = tf.Variable(tf.truncated_normal([hidden_cur_cnt, output], stddev=hidden_stddev / 2))
        ...
        beta = 1e-5
        loss = tf.reduce_mean(tf.square(logits_active - tf_train_y)) + beta * l2_loss
    ...
    with tf.Session(graph=graph) as sess:
        ...
        for step in range(steps):
            ...
            return_w1,return_w2,return_weights,return_b1,return_b2,return_biases,_, l, predictions = sess.run([W1,W2,weights,b1,b2,biases,optimizer,loss,logits],feed_dict={tf_train_x:train_x,tf_train_y:train_y})
            if (step+1) % 1 == 0:
...
def findBestHidden(input,output,train_x,train_y):
    ...
    for i in range(len(s)):
        w,b = tf_better_nn(input,output,s[i],train_x,train_y,steps=1000)
...
def CaptialChart(state_param,Swing):
    train_index1,train_index2,train_index3,train_signal1,train_signal2,train_signal3,test_x,test_signal = EntropyData.ReadDataFromXml('Entropy_data/data15min.xlsx',Swing=Swing,state_param=state_param)
    # train_index1,train_index2,train_index3,train_signal1,train_signal2,train_signal3,test_x,test_signal = EntropyData.ReadDataByTuShare(code='600221',start='2005-11-20',end='2017-11-20',ktype='D',state_param=state_param,Swing=Swing)
    ...
    # Net1W,Net1B = tf_better_nn(19,1,findBestHidden(19,1,x1,train_signal1),x1,train_signal1,layer_cnt=2,keep_prob=0.75)
    # Net2W,Net2B = tf_better_nn(19,1,findBestHidden(19,1,x2,train_signal2),x2,train_signal2,layer_cnt=2,keep_prob=0.75)
    # Net3W,Net3B = tf_better_nn(19,1,findBestHidden(19,1,x3,train_signal3),x3,train_signal3,layer_cnt=2,keep_prob=0.75)
    net1 = tf_learn(x1,train_signal1)
    net2 = tf_learn(x2,train_signal3)
    ...
    plt.figure(1)
    plt.plot(test_signal,'b',f60_singal,'r')
    plt.grid(True)
    f60_operation = np.mat(np.zeros((len(f60_singal),0)))
...
def main():
    ...
    #         best_i = i
    #         break
    quxian = np.cumsum(CaptialChart(max,0.05),axis=0) + 400000
    plt.figure(2)
    plt.plot(quxian)
    plt.title('Capital Line')
    plt.grid(True)
