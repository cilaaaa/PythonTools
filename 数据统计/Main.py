...
cur = conn.cursor()
trade_time = '2018-04-25'
jingzhi = 1.0002346
account = '31680909'
per_capital = [1100000,1000000]
policys = ['Policy511990','Policy511880']
policy_gain = []
policy_jiaoyi = []
policy_chicang = []
policy_nihuigou = []
...
for policy_index,policy in enumerate(policys):
    cur.execute("select * from trade_hist where trade_policyname = '" + policy + "' and trade_time > '"+ trade_time +"' and (trade_account = '" + account + "' or trade_account = '22300000201') order by trade_time asc")
    data = cur.fetchall()
    gain = []
    nihuigou = []
    chicangguoye = []
...
# df.to_excel(writer,sheet_name=policy[6:])
array = np.zeros([len(policy_date[0])-1,5],dtype=object)
leijishouyilv = 0
days = 0
for i,date in enumerate(policy_date[0][:-1]):
    benjin = 0
    shouyi = 0
    ...
    array[i][4] = jingzhi
df = pd.DataFrame(array,index=policy_date[0][:-1],columns=['本金','当日总收益','当日折合年化收益率','累计折合年化收益率','净值'])
