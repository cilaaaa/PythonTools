...
enter_date = '2018-07-28 00:00:00'
...
def get_stock_data(date):
    ...
    sql = "select * from trade_hist where trade_time>='"+date+"'and trade_time<'"+nextday+"'"
...
def deal_data(data):
    ...
    for i in range(len(data)):
        ...
        poundage = copy.copy(total_price * 0.0004)  # 6.25 万四赔率
        if direction == "Buy":
            total_price = -total_price
        if name.split('-')[1] != "USDT":
            total_price = 0
            poundage = np.double(data.loc[i, "入场手续费"]) * 0.2
    ...
    for stock_name in stock_array:
        ...
        save_string = "实盘结果-"+stock_name[13:]+"-"+enter_date.split(" ")[0]+".xls"
        ...
        writer.save()
        excels = file_name("实盘结果-"+stock_name[13:])
        writer2 = pd.ExcelWriter("C:/Users/Cila/Desktop/GainRate/数字货币实盘结果/统计/" + "实盘结果-"+stock_name[13:] + ".xls")
        dates = []
        data = np.array([])
        for excel in excels:
        ...
        for i in range(len(data)):
            ...
            array[i][1] = data[i][1]
            array[i][2] = data[i][2]
        array[len(dates)-1][0] = np.sum(data[:,0],axis=0)
        array[len(dates)-1][1] = np.sum(data[:,1],axis=0)
        array[len(dates)-1][2] = np.sum(data[:,2],axis=0)
